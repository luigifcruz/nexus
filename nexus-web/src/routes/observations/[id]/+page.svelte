<script lang="ts">
    ...
    import { formatTimestamp, formatDuration, formatDateTime } from "$lib/utils/formatters";
    ...
    let calibrationData = $state(Array.from({ length: Math.floor(observationMetrics.antennas * (observationMetrics.antennas - 1) / 2) }, (_, i) => {
        ...
        const timeSeriesData = Array.from({ length: 100 }, (_, j) => {
            ...
            const phase = Math.sin(j * 0.1 + i) * 10 + Math.random() * 5;
            const amplitude = correlation * 100 + Math.sin(j * 0.05 + i * 0.3) * 15 + Math.random() * 8;
            return {
                timestamp: new Date(time),
                phase: phase,
                amplitude: Math.max(0, amplitude),
                correlation: correlation + Math.sin(j * 0.02) * 0.1 + Math.random() * 0.05
            };
        });
    ...
    let metricsHistory = $derived(transformObservationMetrics(dataStore.getObservationMetrics(observationId)));
    ...
    const stellineEdges = [
        { id: "e1", source: "network-op", target: "packet-sorter", style: "stroke: #666; stroke-width: 2px;" },
        { id: "e2", source: "packet-sorter", target: "kurtosis", style: "stroke: #666; stroke-width: 2px;" },
        { id: "e3", source: "kurtosis", target: "beamformer", style: "stroke: #666; stroke-width: 2px;" },
        { id: "e4", source: "beamformer", target: "frbnn", style: "stroke: #666; stroke-width: 2px;" },
        { id: "e5", source: "kurtosis", target: "correlator", style: "stroke: #666; stroke-width: 2px;" },
        { id: "e6", source: "correlator", target: "hdf5", style: "stroke: #666; stroke-width: 2px;" }
    ];
    ...
    const postProcessEdges = [
        { id: "p1", source: "end-trigger", target: "copy-assets", style: "stroke: #666; stroke-width: 2px;" },
        { id: "p2", source: "copy-assets", target: "hdd-storage", style: "stroke: #666; stroke-width: 2px;" }
    ];
            ...
            try {
                ...
                if (obs) {
                    ...
                    if (obs.replicants && obs.replicants.length > 0) {
                        replicant = dataStore.getReplicantById(obs.replicants[0]);
                    }
                } else {
    ...
    const totalDuration = $derived(routineData.endTime.getTime() - routineData.startTime.getTime());
    const currentOffset = $derived(routineData.currentTime.getTime() - routineData.startTime.getTime());
    const currentPercent = $derived(Math.max(0, Math.min(100, (currentOffset / totalDuration) * 100)));

    // Mock replicants for this observation
    const observationReplicants = dataStore.replicants.filter(r => r.status === "running").slice(0, observationMetrics.replicants);

    // Helper to check if an event is currently active
    function isEventActive(event: any): boolean {
        return routineData.currentTime >= event.startTime && routineData.currentTime <= event.endTime;
    }
</script>
                            ...
                            <div class="flex items-center gap-2">
                                <Tooltip.Provider>
                                    <Tooltip.Root>
                                        ...
                                        <Tooltip.Content>
                                            <p>Status: {getStatusLabel(observation.status)}</p>
                                        </Tooltip.Content>
                                    </Tooltip.Root>
                                </Tooltip.Provider>
            ...
            <Tabs.Root bind:value={activeTab} class="space-y-6">
                ...
                <Tabs.Content value="routine" class="space-y-6">
                    <Card.Root>
                        ...
                        <Card.Content>
                            <div class="space-y-6">
                                <!-- Timeline Container -->
                                <div class="relative h-20 bg-muted/20 rounded-lg overflow-hidden">
                                    <!-- Timeline Background -->
                                    <div class="absolute inset-0 bg-gradient-to-r from-blue-100 via-green-100 to-purple-100 dark:from-blue-950 dark:via-green-950 dark:to-purple-950"></div>

                                    <!-- Event Blocks -->
                                    {#each routineData.events as event}
                                        {@const eventStart = event.startTime.getTime() - routineData.startTime.getTime()}
                                        {@const eventDuration = event.endTime.getTime() - event.startTime.getTime()}
                                        {@const leftPercent = (eventStart / totalDuration) * 100}
                                        {@const widthPercent = (eventDuration / totalDuration) * 100}
                                        {@const isTarget = event.type === 'target'}
                                        {@const isSteering = event.type === 'steering'}
                                        ...
                                        {:else}
                                            ...
                                            <div
                                                ...
                                                style="left: {leftPercent}%; width: {widthPercent}%; background: {isTarget ? 'linear-gradient(135deg, #3b82f6 0%, #1e40af 100%)' : 'linear-gradient(135deg, #6b7280 0%, #374151 100%)'};"
                                                title="{event.name}{event.ra ? ` - RA: ${event.ra}, Dec: ${event.dec}, Freq: ${event.frequency}` : event.description ? ` - ${event.description}` : ''}"
                                            >
                                                {#if isTarget}
                                                    <div class="p-2 text-white text-center">
                                                        <div class="text-sm font-semibold">{event.name}</div>
                                                        <div class="text-xs font-mono opacity-70">{event.frequency}</div>
                                                    </div>
                                                {/if}
                                            </div>
                                        {/if}
                                ...
                                <!-- Timeline Footer -->
                                <div class="flex justify-between items-center text-sm text-muted-foreground font-mono">
                                    <span>Start: {routineData.startTime.toLocaleString()}</span>
                                    <span>End: {routineData.endTime.toLocaleString()}</span>
                                </div>
                                    ...
                                    <Table.Root>
                                        ...
                                        <Table.Body>
                                            {#each routineData.events as event, i}
                                                {@const isCurrent = routineData.currentTime >= event.startTime && routineData.currentTime <= event.endTime}
                                                {@const duration = formatTargetDuration(event.startTime, event.endTime)}
                                                {@const isTarget = event.type === 'target'}
                                                {@const isSteering = event.type === 'steering'}
                                                {@const isWarning = event.type === 'warning'}
                                                {@const shouldIndent = !isTarget && (isSteering || isWarning)}
                                                <Table.Row>
                                                     <Table.Cell class="font-medium">
                                                         <div class="flex items-center gap-2 {shouldIndent ? 'ml-6' : ''}">
                                                             {#if isTarget}
                                                                 <TargetIcon class="h-4 w-4 text-blue-500" />
                                                             {:else if isSteering}
                                                                 <SettingsIcon class="h-4 w-4 text-gray-500" />
                                                             {:else if isWarning}
                                                                 <WarningIcon class="h-4 w-4 text-yellow-500" />
                                                             {/if}
                                                             <span class="font-mono text-sm text-foreground">{event.name}</span>
                                                             {#if isCurrent && isTarget}
                                                                 <Badge variant="default" class="text-xs bg-green-600 hover:bg-green-700 ml-2">
                                                                     Active
                                                                 </Badge>
                                                             {/if}
                                                         </div>
                                                    ...
                                                    <Table.Cell>
                                                        <span class="font-mono text-sm text-foreground">{event.ra || '—'}</span>
                                                    </Table.Cell>
                                                    <Table.Cell>
                                                        <span class="font-mono text-sm text-foreground">{event.dec || '—'}</span>
                                                    </Table.Cell>
                                                    <Table.Cell>
                                                        <span class="font-mono text-sm text-foreground">{event.frequency || '—'}</span>
                                                    </Table.Cell>
                                                    <Table.Cell>
                                                        <span class="text-xs text-muted-foreground">
                                                            {event.startTime.toLocaleTimeString()} - {event.endTime.toLocaleTimeString()} ({duration})
                                                        </span>
                                                    </Table.Cell>
                                                </Table.Row>
                                            {/each}
                ...
                <Tabs.Content value="resources" class="space-y-6">
                    ...
                    <Card.Root>
                        ...
                        <Card.Content>
                            <ReplicantTable replicants={observationReplicants} />
                        </Card.Content>
                    </Card.Root>
                                ...
                                <Table.Root>
                                    <Table.Header>
                                        <Table.Row>
                                            ...
                                            <Table.Head class="text-center">Azimuth</Table.Head>
                                            <Table.Head class="text-center">Elevation</Table.Head>
                                            <Table.Head class="text-center">Feed Temp (K)</Table.Head>
                                            <Table.Head class="text-center">Power (dBm)</Table.Head>
                                        </Table.Row>
                                    </Table.Header>
                                    <Table.Body>
                                        {#each Array(observationMetrics.antennas) as _, i}
                                            ...
                                            {@const azError = (Math.random() * 0.2).toFixed(2)}
                                            {@const elError = (Math.random() * 0.2).toFixed(2)}
                                            {@const signalStrength = -120 + Math.random() * 80}
                                            {@const feedTemp = (1 + Math.random() * 4).toFixed(1)}
                                            {@const station = Math.floor(i / 3) + 1}
                                            {@const antennaLetter = String.fromCharCode(65 + (i % 3))}
                                            <Table.Row>
                                                <Table.Cell class="font-medium">
                                                    <div class="flex items-center gap-2">
                                                        <SatelliteDishIcon class="h-5 w-5 {isOnline ? 'text-green-600' : 'text-red-600'}" />
                                                        <span class="font-mono text-sm">{station}{antennaLetter}</span>
                                                    </div>
                                                </Table.Cell>
                                                <Table.Cell class="text-center">
                                                    <div class="flex flex-col items-center">
                                                        <span class="font-mono text-sm">{isOnline ? azimuth + "°" : "—"}</span>
                                                        {#if isOnline}
                                                            <span class="font-mono text-xs text-muted-foreground">±{azError}°</span>
                                                        {/if}
                                                    </div>
                                                </Table.Cell>
                                                <Table.Cell class="text-center">
                                                    <div class="flex flex-col items-center">
                                                        <span class="font-mono text-sm">{isOnline ? elevation + "°" : "—"}</span>
                                                        {#if isOnline}
                                                            <span class="font-mono text-xs text-muted-foreground">±{elError}°</span>
                                                        {/if}
                                                    </div>
                                                </Table.Cell>
                                                <Table.Cell class="text-center">
                    ...
                    <Card.Root>
                        ...
                        <Card.Content>
                            <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                <div class="flex flex-col items-center gap-2 p-4 rounded-lg bg-muted/30">
                                    <span class="text-2xl font-bold text-green-600">
                                        {(calibrationData.reduce((sum, b) => sum + parseFloat(b.correlation), 0) / calibrationData.length).toFixed(3)}
                                    </span>
                                    <span class="text-xs text-muted-foreground text-center">Average Correlation</span>
                                </div>
                                <div class="flex flex-col items-center gap-2 p-4 rounded-lg bg-muted/30">
                                    <span class="text-2xl font-bold text-blue-600">
                                        {(calibrationData.reduce((sum, b) => sum + parseFloat(b.phaseStability), 0) / calibrationData.length).toFixed(3)}
                                    </span>
                                    <span class="text-xs text-muted-foreground text-center">Phase Stability</span>
                                </div>
                                <div class="flex flex-col items-center gap-2 p-4 rounded-lg bg-muted/30">
                                    <span class="text-2xl font-bold text-purple-600">
                                        {calibrationData.filter(b => parseFloat(b.correlation) > 0.85).length}
                                    </span>
                                    <span class="text-xs text-muted-foreground text-center">High Quality Baselines</span>
                                </div>
                                <div class="flex flex-col items-center gap-2 p-4 rounded-lg bg-muted/30">
                                    <span class="text-2xl font-bold text-orange-600">
                                        {((calibrationData.reduce((sum, b) => sum + parseFloat(b.rmsNoise), 0) / calibrationData.length) * 1000).toFixed(1)}
                                    </span>
                                    <span class="text-xs text-muted-foreground text-center">RMS Noise (mJy)</span>
                                </div>
                            </div>
                        </Card.Content>
                    </Card.Root>
                                    ...
                                    <Card.Root>
                                        <Card.Header class="pb-2">
                                            <div class="flex items-center justify-between">
                                                <Card.Title class="text-sm font-mono text-foreground">
                                                    {baseline.baselineId}
                                                </Card.Title>
                                                <QualityBadge value={parseFloat(baseline.correlation)} class="text-xs" />
                                            </div>
                                            <div class="text-xs text-muted-foreground">
                                                Correlation: {baseline.correlation} | Phase: {baseline.phaseStability}
                                            </div>
                                        </Card.Header>
                                        <Card.Content>
                                            <div class="space-y-2">
                                                <Chart.Container config={{
                                                    amplitude: { label: "Amplitude", color: "#3b82f6" }
                                                }} class="h-20 w-full">
                                                    <AreaChart
                                                        data={baseline.timeSeriesData.slice(-30)}
                                                        x="timestamp"
                                                        series={[
                                                            {
                                                                key: "amplitude",
                                                    ...
                                                    >
                                                        {#snippet marks({ series, getAreaProps })}
                                                            <defs>
                                                                <linearGradient id="fillAmplitude{baseline.baselineId}" x1="0" y1="0" x2="0" y2="1">
                                                                    <stop offset="5%" stop-color="#3b82f6" stop-opacity={0.3} />
                                                                    <stop offset="95%" stop-color="#3b82f6" stop-opacity={0.05} />
                                                                </linearGradient>
                                                            </defs>
                                                            {#each series as s, i (s.key)}
                                                                <Area {...getAreaProps(s, i)} fill="url(#fillAmplitude{baseline.baselineId})" />
                                                            {/each}
                                                        {/snippet}
                                                    </AreaChart>
                                                </Chart.Container>
                                                <div class="grid grid-cols-2 gap-2 text-xs">
                                                    <div class="flex justify-between">
                                                        ...
                                                        <span class="font-mono text-foreground">{baseline.rmsNoise}</span>
                                                    </div>
                                                    <div class="flex justify-between">
                                                        <span class="text-muted-foreground">Amp Ratio:</span>
                                                        <span class="font-mono text-foreground">{baseline.amplitudeRatio}</span>
                                                    </div>
                                                </div>
                                            </div>
                                        </Card.Content>
                ...
                <Tabs.Content value="stelline" class="space-y-6">
                    <Card.Root>
                        ...
                        <Card.Content>
                            <div class="w-full border rounded-lg bg-gray-50 dark:bg-gray-900" style="aspect-ratio: 16/10;">
                                <SvelteFlow
                                    id="stelline-flow"
                                    nodes={stellineNodes}
                                    edges={stellineEdges}
