<script lang="ts">
    ...
    // Test data
    const statusValues = [
        "running", "online", "stopped", "completed", "errored", "error", "failed",
        "standby", "starting", "stopping", "maintenance", "scheduled", "pending",
        "offline", "unknown"
    ];
    ...
    <div class="px-4 lg:px-6 space-y-8">
        ...
        <Card.Root>
            <Card.Header>
                ...
                <Card.Description>Unified transparent badge system with consistent styling</Card.Description>
            </Card.Header>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
                        {#each statusValues as status}
                            <div class="flex flex-col items-center gap-2 p-3 rounded-lg border bg-muted/20">
                                <StatusBadge {status} />
                            ...
                            <Table.Body>
                                {#each sampleTableData as row}
                                    <Table.Row class="hover:bg-muted/50">
                                        ...
                                        <Table.Cell>
                                            <StatusBadge status={row.status} showIcon />
                                        </Table.Cell>
                                    </Table.Row>
                                {/each}
                            </Table.Body>
        ...
        <Card.Root>
            <Card.Header>
                ...
                <Card.Description>For correlation analysis and quality indicators</Card.Description>
            </Card.Header>
            <Card.Content class="space-y-6">
                <div>
                    ...
                    <div class="grid grid-cols-3 gap-4">
                        {#each correlationSamples as sample}
                            <div class="flex flex-col items-center gap-2 p-3 rounded-lg border bg-muted/20">
                                ...
                                <code class="text-xs text-muted-foreground">{sample.correlation}</code>
                                <span class="text-xs text-muted-foreground">{sample.id}</span>
                            </div>
                        {/each}
                    </div>
                </div>
                ...
                <div>
                    ...
                    <div class="grid grid-cols-4 gap-4">
                        <div class="flex flex-col items-center gap-2 p-3 rounded-lg border bg-muted/20">
                            ...
                            <code class="text-xs text-muted-foreground">0.85 (default)</code>
                        </div>
                        <div class="flex flex-col items-center gap-2 p-3 rounded-lg border bg-muted/20">
                            <QualityBadge value={0.85} thresholds={{ excellent: 0.95, good: 0.85 }} />
                            <code class="text-xs text-muted-foreground">0.85 (strict)</code>
                        </div>
                        <div class="flex flex-col items-center gap-2 p-3 rounded-lg border bg-muted/20">
                            <QualityBadge value={0.75} thresholds={{ excellent: 0.8, good: 0.6 }} />
                            <code class="text-xs text-muted-foreground">0.75 (lenient)</code>
                        </div>
                        <div class="flex flex-col items-center gap-2 p-3 rounded-lg border bg-muted/20">
                            <QualityBadge value={0.95} class="text-xs" />
                            <code class="text-xs text-muted-foreground">0.95 (small)</code>
                        </div>
                    </div>
                </div>
            </Card.Content>
        ...
        <Card.Root>
            <Card.Header>
                ...
                <Card.Description>Standard badges for labels, counts, and indicators</Card.Description>
            </Card.Header>
            <Card.Content class="space-y-6">
                <div>
                    ...
                    <div class="flex flex-wrap gap-2">
                        ...
                        <Badge variant="outline" class="font-mono">N6MCC51544</Badge>
                    </div>
                </div>
        ...
        <Card.Root>
            <Card.Header>
                ...
                <Card.Description>Badge system rules and color meanings</Card.Description>
            </Card.Header>
            <Card.Content>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                        ...
                        <ul class="space-y-1 text-sm text-muted-foreground">
                            <li><strong class="text-foreground">Green:</strong> running, online</li>
                            <li><strong class="text-foreground">Blue:</strong> completed, stopped</li>
                            <li><strong class="text-foreground">Red:</strong> errored, error, failed</li>
                            <li><strong class="text-foreground">Yellow:</strong> standby, starting, stopping, maintenance, scheduled, pending</li>
                            <li><strong class="text-foreground">Gray:</strong> offline, unknown</li>
                        </ul>
                    </div>
                    <div>
                        <h4 class="font-semibold mb-2">Design Rules</h4>
                        <ul class="space-y-1 text-sm text-muted-foreground">
                            <li><strong class="text-foreground">All transparent:</strong> outline variant only</li>
                            <li><strong class="text-foreground">Consistent colors:</strong> semantic meaning</li>
                            <li><strong class="text-foreground">Auto capitalization:</strong> text formatting</li>
                            <li><strong class="text-foreground">Optional icons:</strong> showIcon prop</li>
                        </ul>
                    </div>
                </div>
                ...
                <div class="mt-6 pt-4 border-t">
                    ...
                    <div class="bg-muted/50 p-3 rounded text-sm font-mono space-y-1">
