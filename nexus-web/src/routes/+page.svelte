<script lang="ts">
    ...
    let recentObservations = $derived(
    ...
    let observationHealth = $derived.by(() => {
        ...
        const recentFailedCount = dataStore.observations.filter((obs) => {
            if (obs.status !== "errored" && obs.status !== "failed")
                return false;
        ...
        if (recentFailedCount > 0)
            return { level: "Degraded", color: "orange" };
    ...
    function getHealthClasses(health: { level: string; color: string }) {
        const colorMap = {
            ...
            red: {
                ...
                border: "border-2 border-red-500/20",
        ...
        const colors =
            colorMap[health.color as keyof typeof colorMap] || colorMap.green;
...
<div class="flex flex-col gap-6 py-6">
    ...
    <div class="px-4 lg:px-6">
        <div class="grid grid-cols-4 gap-3">
            <!-- System Health -->
            <Card.Root
                class="{replicantClasses.border} {replicantClasses.gradient}"
            >
                <Card.Header class="pb-1 pt-1 relative z-10">
                    <div class="flex items-center">
                        <div class="flex items-center gap-3">
                            <div
                                class="p-2 rounded-lg {replicantClasses.iconBg}"
                            >
                                <ServerIcon
                                    class="h-5 w-5 {replicantClasses.iconColor}"
                                />
                            </div>
                            <div>
                                <Card.Description
                    ...
                    <div class="flex flex-row space-x-2">
                        <Badge variant="success"
                            >{onlineReplicants.length} Healthy</Badge
                        >
                    </div>
                </Card.Header>
            </Card.Root>
            ...
            <!-- Active Observations -->
            <Card.Root
                class="{observationClasses.border} {observationClasses.gradient}"
            >
                <Card.Header class="pb-1 pt-1 relative z-10">
                    <div class="flex items-center">
                        <div class="flex items-center gap-3">
                            <div
                                class="p-2 rounded-lg {observationClasses.iconBg}"
                            >
                                <EyeIcon
                                    class="h-5 w-5 {observationClasses.iconColor}"
                                />
                            </div>
                            <div>
                                <Card.Description
                    ...
                    <div class="flex flex-row space-x-2">
                        ...
                        <Badge variant="success"
                            >{scheduledCount} Scheduled</Badge
                        >
                    </div>
                </Card.Header>
            </Card.Root>
            ...
            <!-- Total Bandwidth -->
            <Card.Root
                class="{bandwidthClasses.border} {bandwidthClasses.gradient}"
            >
                <Card.Header class="pb-1 pt-1 relative z-10">
                    <div class="flex items-center">
                        <div class="flex items-center gap-3">
                            <div
                                class="p-2 rounded-lg {bandwidthClasses.iconBg}"
                            >
                                <NetworkIcon
                                    class="h-5 w-5 {bandwidthClasses.iconColor}"
                                />
                            </div>
                            <div>
                                <Card.Description
                    ...
                    <Card.Title class="text-3xl font-bold tabular-nums pt-2">
                        {dataStore.systemMetrics.totalBandwidth}
                        <span class="text-xl font-medium text-muted-foreground"
                            >Mbps</span
                        >
                    </Card.Title>
                    <div
                        class="text-sm text-muted-foreground italic tabular-nums pt-0"
                    >
                        of 12.8 Tbps
                    </div>
                </Card.Header>
            ...
            <!-- Storage Usage -->
            <Card.Root
                class="{storageClasses.border} {storageClasses.gradient}"
            >
                <Card.Header class="pb-1 pt-1 relative z-10">
                    <div class="flex items-center">
                        <div class="flex items-center gap-3">
                            <div class="p-2 rounded-lg {storageClasses.iconBg}">
                                <DatabaseIcon
                                    class="h-5 w-5 {storageClasses.iconColor}"
                                />
                            </div>
                            <div>
                                <Card.Description
                    ...
                    <Card.Title class="text-3xl font-bold tabular-nums pt-2">
                        {formatBytes(
                            (dataStore.systemMetrics.usedStorage || 0) *
                                1024 *
                                1024 *
                                1024,
                        )}
    ...
    <div class="px-4 lg:px-6">
        <div class="flex items-center justify-between mb-4">
            ...
            <Button variant="ghost" size="sm" onclick={navigateToObservations}>
                View All
            </Button>
        </div>
